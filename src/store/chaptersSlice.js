...
const chaptersSlice = createSlice({
  ...
  reducers: {
    ...
    updateChapterProgress: (state, action) => {
      const { chapter, questionSolved, status } = action.payload;
      const chapterIndex = state.data.findIndex(c => c.chapter === chapter);
      if (chapterIndex !== -1) {
        state.data[chapterIndex].questionSolved = questionSolved;
        state.data[chapterIndex].status = status;
      }
    }
  }
});

export const { setActiveSubject, updateChapterProgress } = chaptersSlice.actions;
